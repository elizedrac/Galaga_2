...
public class RunGalaga implements Runnable {
    public void keyClose(int k, GameCourt court, JFrame frame) {
        court.setKeys(k);
    ...
    public void run() {
        ...
        close.addActionListener(e -> this.keyClose(KeyEvent.VK_UP, court, frame1));
        control_panel1.add(close);

        final JButton changeKeys = new JButton("W Shoot Key");
        changeKeys.addActionListener(e -> this.keyClose(KeyEvent.VK_W, court, frame1));
        control_panel1.add(changeKeys);


        //fix (may pause/unpause feature)
